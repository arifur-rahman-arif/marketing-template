...
const Dropdown = ({ title, onClick, isOpen, options, onChange, type, closeDropdown }: Props) => {
    ...
    return (
        <div className="relative grid w-[16.5rem]" ref={outsideRef}>
            ...
            {/* Items */}
            <div
                className={`absolute left-0 top-full z-[3] mt-2 w-full overflow-hidden transition-all duration-700 ${isOpen ? 'max-h-[20rem]' : 'max-h-0'}`}
            >
                <div className="grid gap-2 rounded-small border border-solid border-gray-20 bg-white px-4 py-2">
